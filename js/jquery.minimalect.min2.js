...
;(function ( $, window, document, undefined ) {
	...
	defaults = {
		...
		// messages
		placeholder: "Select ", // default placeholder when nothing is selected
		...
		// callbacks
		beforeinit: function(){}, // called before minimalect2 is initialized
		afterinit: function(){}, // called right after minimalect2 is initialized
	...
	Plugin.prototype = {
			...
			if(window.MutationObserver){
				...
				this.observer = new MutationObserver(function( mutations ) {
					...
					if(mutations.length > 0){
						...
						if(m.options.debug) console.log("minimalect2 detected a DOM change for ", m.element);
					}
				});
		...
		_showChoices: function(cb){
			...
			if (!wr.hasClass(op.class_active)){
				...
				// close all other open minimalect1s
			...
			if(op.ajax) {
				...
				$.post(op.ajax, {"q": this.input.text()})
					.success(function(data){
						...
						if(op.debug) console.log("minimalect2 received ", data, " for query '"+m.input.text()+"' in ", m.element);
						...
						if(data.length) {
							...
							if(op.debug) console.log("minimalect2 didn't find any results for '"+m.input.text()+"' from ", m.element);
					...
					.error(function(data){
						...
						wr.find("."+op.class_empty).show();
						// tell the console if debug mode is on
						if(op.debug) console.error("minimalect2's AJAX query failed for ", m.element, " - came back with ", data);
					});
			} else {
				...
				if(!this.items.not("."+op.class_hidden+", ."+op.class_empty).length) {
					wr.find("."+op.class_empty).show();
					// tell the console if debug mode is on
					if(op.debug) console.log("minimalect2 didn't find any results for '"+this.input.text()+"' from ", this.element);
					// callback, no results found
		...
		// uninit minimalect2
		destroy: function(){
			// remove (and automatically unbind) all minimalect2 stuff
			this.wrapper.remove();
			...
			if(this.options.debug) console.log("minimalect2 destroyed for ", this.element);
		},

		// update minimalect2's choice from the original select
		update: function(){
			// reparse the select
	...
	$.fn[pluginName2] = function ( options, argument ) {
		return this.each(function () {
			if ($.isFunction(Plugin.prototype[options]) && options.charAt(0) != "_") {
				if(arguments.length == 1)
					$.data(this, 'plugin_' + pluginName2)[options]();
				else
					$.data(this, 'plugin_' + pluginName2)[options](argument);
			} else if (!$.data(this, "plugin_" + pluginName2)) {
				$.data(this, "plugin_" + pluginName2, new Plugin( this, options ));
			}
		});
	};
	
	var pluginName11 = "minimalect11",
	defaults = {
		// settings
		theme: "", // name of the theme used
		reset: false,
		...
		// messages
		placeholder: "Experience", // default placeholder when nothing is selected
		empty: "No results match your keyword.", // error message when nothing matches the filter search term
		error_message: "There was a problem with the request.", // error message when the AJAX call fails
	...
	$.fn[pluginName11] = function ( options, argument ) {
		return this.each(function () {
			if ($.isFunction(Plugin.prototype[options]) && options.charAt(0) != "_") {
				if(arguments.length == 1)
					$.data(this, 'plugin_' + pluginName11)[options]();
				else
					$.data(this, 'plugin_' + pluginName11)[options](argument);
			} else if (!$.data(this, "plugin_" + pluginName11)) {
				$.data(this, "plugin_" + pluginName11, new Plugin( this, options ));
			}
		});
	};
