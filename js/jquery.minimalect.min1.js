...
;(function ( $, window, document, undefined ) {
	...
	defaults = {
		...
		// messages
		placeholder: "Education ", // default placeholder when nothing is selected
		...
		// callbacks
		beforeinit: function(){}, // called before minimalect1 is initialized
		afterinit: function(){}, // called right after minimalect1 is initialized
	...
	Plugin.prototype = {
			...
			if(window.MutationObserver){
				...
				this.observer = new MutationObserver(function( mutations ) {
					...
					if(mutations.length > 0){
						...
						if(m.options.debug) console.log("minimalect1 detected a DOM change for ", m.element);
					}
				});
			...
			if(op.ajax) {
				...
				$.post(op.ajax, {"q": this.input.text()})
					.success(function(data){
						...
						if(op.debug) console.log("minimalect1 received ", data, " for query '"+m.input.text()+"' in ", m.element);
						...
						if(data.length) {
							...
							if(op.debug) console.log("minimalect1 didn't find any results for '"+m.input.text()+"' from ", m.element);
					...
					.error(function(data){
						...
						wr.find("."+op.class_empty).show();
						// tell the console if debug mode is on
						if(op.debug) console.error("minimalect1's AJAX query failed for ", m.element, " - came back with ", data);
					});
			} else {
				...
				if(!this.items.not("."+op.class_hidden+", ."+op.class_empty).length) {
					wr.find("."+op.class_empty).show();
					// tell the console if debug mode is on
					if(op.debug) console.log("minimalect1 didn't find any results for '"+this.input.text()+"' from ", this.element);
					// callback, no results found
		...
		// uninit minimalect1
		destroy: function(){
			// remove (and automatically unbind) all minimalect1 stuff
			this.wrapper.remove();
			...
			if(this.options.debug) console.log("minimalect1 destroyed for ", this.element);
		},

		// update minimalect1's choice from the original select
		update: function(){
			// reparse the select
	...
	$.fn[pluginName1] = function ( options, argument ) {
		return this.each(function () {
			if ($.isFunction(Plugin.prototype[options]) && options.charAt(0) != "_") {
				if(arguments.length == 1)
					$.data(this, 'plugin_' + pluginName1)[options]();
				else
					$.data(this, 'plugin_' + pluginName1)[options](argument);
			} else if (!$.data(this, "plugin_" + pluginName1)) {
				$.data(this, "plugin_" + pluginName1, new Plugin( this, options ));
			}
		});
	};
